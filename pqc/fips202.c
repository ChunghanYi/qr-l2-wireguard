...
void shake128_inc_init(shake128incctx *state) {
    state->ctx = kmalloc(PQC_SHAKEINCCTX_BYTES, GFP_KERNEL);
    if (state->ctx == NULL) {
		return;
    }
...
void shake128_inc_ctx_clone(shake128incctx *dest, const shake128incctx *src) {
    dest->ctx = kmalloc(PQC_SHAKEINCCTX_BYTES, GFP_KERNEL);
    if (dest->ctx == NULL) {
		return;
    }
...
void shake128_absorb(shake128ctx *state, const uint8_t *input, size_t inlen) {
    state->ctx = kmalloc(PQC_SHAKECTX_BYTES, GFP_KERNEL);
    if (state->ctx == NULL) {
		return;
    }
...
void shake128_ctx_clone(shake128ctx *dest, const shake128ctx *src) {
    dest->ctx = kmalloc(PQC_SHAKECTX_BYTES, GFP_KERNEL);
    if (dest->ctx == NULL) {
		return;
    }
