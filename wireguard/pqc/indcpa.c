...
#else /* added by chunghan.yi@gmail.com */
void PQCLEAN_KYBER768_CLEAN_indcpa_keypair(uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES],
        uint8_t sk[KYBER_INDCPA_SECRETKEYBYTES]) {
    unsigned int i;
    uint8_t buf[2 * KYBER_SYMBYTES];
    ...
    const uint8_t *noiseseed = buf + KYBER_SYMBYTES;
    uint8_t nonce = 0;
	polyvec *a = kmalloc(sizeof(polyvec) * KYBER_K, GFP_KERNEL);
	polyvec *e = kmalloc(sizeof(polyvec), GFP_KERNEL);
	polyvec *pkpv = kmalloc(sizeof(polyvec), GFP_KERNEL);
	polyvec *skpv = kmalloc(sizeof(polyvec), GFP_KERNEL);

	if ((a == NULL) || (e == NULL) || (pkpv == NULL) || (skpv == NULL)) return;
...
void PQCLEAN_KYBER768_CLEAN_indcpa_enc(uint8_t c[KYBER_INDCPA_BYTES],
	...
	polyvec *sp = kmalloc(sizeof(polyvec), GFP_KERNEL);
	polyvec *pkpv = kmalloc(sizeof(polyvec), GFP_KERNEL);
	polyvec *ep = kmalloc(sizeof(polyvec), GFP_KERNEL);
	polyvec *at = kmalloc(sizeof(polyvec) * KYBER_K, GFP_KERNEL);
	polyvec *b = kmalloc(sizeof(polyvec), GFP_KERNEL);

    poly *v = kmalloc(sizeof(poly), GFP_KERNEL);
    poly *k = kmalloc(sizeof(poly), GFP_KERNEL);
    poly *epp = kmalloc(sizeof(poly), GFP_KERNEL);

	if ((sp == NULL) || (pkpv == NULL) || (ep == NULL) || (at == NULL) || (b == NULL)) return;
	if ((v == NULL) || (k == NULL) || (epp == NULL)) return;
...
void PQCLEAN_KYBER768_CLEAN_indcpa_dec(uint8_t m[KYBER_INDCPA_MSGBYTES],
	...
	poly *v = kmalloc(sizeof(poly), GFP_KERNEL);
	poly *mp = kmalloc(sizeof(poly), GFP_KERNEL);

	if ((b == NULL) || (skpv == NULL)) return;
	if ((v == NULL) || (mp == NULL)) return;
