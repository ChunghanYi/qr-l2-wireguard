...
unsigned int l2wg_get_packet_length(struct sk_buff *skb)
{
	...
	if (skb->protocol == htons(ETH_P_IP)) {
		...
		if (((struct llc_hdr*)skb_network_header(skb))->dsap == 0xaa &&
...
bool l2wg_check_packet_length_internal(struct sk_buff *skb, struct net_device *dev,
		bool wrapped)
{
	bool valid;
	u8 slow_sub_type;
...
#ifdef L2_WIREGUARD
	routed_peer = wg->allowedips_lookup_src(&peer->device->peer_allowedips, skb);
#else
